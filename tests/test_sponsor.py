...
class SponsorClientTest(unittest.TestCase):
    def test_missing_service_url_is_reported_as_unavailable(self) -> None:
        with patch.dict(os.environ, {SPONSOR_API_ENV: ""}, clear=False):
            with self.assertRaisesRegex(SponsorUnavailable, "尚未配置"):
                SponsorClient.from_environment()
    ...
    def test_create_order_uses_yungouos_provider_and_validates_qr_url(self) -> None:
        ...
        order = client.create_order("6", app_version="0.5.3")

        self.assertEqual(order.order_id, "order-1")
        method, url, kwargs = session.calls[0]
        self.assertEqual((method, url), ("POST", "https://pay.example.com/api/sponsor/orders"))
    ...
    def test_download_qr_requires_an_image(self) -> None:
        client = SponsorClient(
            "https://pay.example.com/api/sponsor",
            session=_Session([_Response(content=b"<html>", content_type="text/html")]),
        )
    ...
    def test_request_network_failure_is_friendly(self) -> None:
        ...
        client = SponsorClient("https://pay.example.com/api/sponsor", session=FailingSession())
        with self.assertRaisesRegex(SponsorUnavailable, "暂时不可用"):
            client.create_order("6.00", app_version="0.5.3")
