...
- `观看线程`：用户填写多少就严格建立多少条独立 `x25Kn` 长会话；当前最多支持 `100` 路。多路可快速追平 B 站已经释放的直播分钟，但无法超过同一账号当前可累计的直播时长上限。界面始终以 `totalv2` 的 `indicators[0].cur_value` 作为 B 站真实入账口径。
...
- **账号异步结果隔离**：自动登录只更新发起时的账号；切换、删除或修改账号后，不会覆盖另一个账号的 Cookie。关闭软件会取消获取并清理自动登录浏览器。
- **故障降级与重试**：网络超时不再误报 Cookie 失效；活动接口异常或空响应会正确降级为手动领奖，观看连接继续运行。
- **完整检查停止状态**：等待预算耗尽后仍检查全部后台线程，上一轮网络请求未退出时不启动重叠的观看或领奖流程。
- **减少日志重绘**：隐藏日志不再重绘当前视图，新增日志增量追加，队列分批处理，让出界面事件循环。
- **赞助金额独立就绪**：各金额独立预取、独立返回；缓存限制为 16 项并回收过期项，网络连接及时关闭，失败信息不再持有异常堆栈。
- **实绩提示纠正**：只展示实际观测倍率；零增长显示“未见新增时长”，不再据此推断“已追平直播时长上限”。保留有效观看会话。
- **服务端订单池清理**：清理超过一天、未分配给用户的失败/过期预建订单，保留用户订单和支付记录；此项需要更新北京赞助服务后生效。

## v0.5.30 新增与修复

- **确认“先快后慢”不是线程停止**：同账号分别完成 20 路后台会话、10 个本地浏览器页面和 10 个不同直播间的 `totalv2` 实测。多路启动时可快速补入已经释放的分钟，追平后统一回到约 `1x`，与当天实际已播时长上限一致。
- **取消错误的实绩停滞重建**：`totalv2` 停滞不再拆毁全部有效长会话；只有 `x25Kn` 请求本身失败才重建对应路由，避免约 90 秒后出现“前几分钟快、后续不计”。
- **恢复竞品同款长会话链路**：每路固定执行 `roomEntryAction → getInfoByRoom → x25Kn/E/X`，使用独立 HTTP client、`AUTO...` 会话身份和页面 UUID，并按返回间隔持续续期。
- **识别当前直播时长上限**：当至少 80% 路由健康、B 站实绩回落到约 `1x` 时，连接详情明确显示“已追平当前直播时长上限，连接保持中”，不再把服务端裁剪误报成断流。
...
- **优化 UI 首帧速度**：圆角卡片和按钮改用 Tk Canvas 矢量绘制，取消首屏反复创建 3 倍分辨率位图、高斯模糊和 PhotoImage 上传。
- **消除启动时界面跳动**：主窗口先在隐藏状态完成布局，再一次性显示完整界面，避免卡片和按钮逐块出现。
- **避免后台任务争抢首帧**：赞助服务网络预热延后到主界面显示后执行，不再与 Tk 首次布局竞争 CPU 和 I/O。
- **精简发布包冷启动**：不再打包未使用的 Selenium 浏览器驱动、Pillow 插件和 numpy；EXE 从 57.83 MiB 降至 42.17 MiB，并新增 Chrome/Edge 驱动冻结包自检。
- **新增启动性能回归**：自动检查主界面使用轻量矢量图元、赞助预热未在首帧前启动，防止后续改动重新引入卡顿。
...
- **弹窗固定居中**：帮助、关于、连接详情、删除确认和错误提示都以软件主窗口为中心，兼容多显示器坐标。
- **修复默认界面吞字**：运行状态表格横跨整张卡片，按钮按文字计算最小宽度。
...
- **重做应用内弹窗**：帮助、详情、关于和结果提示统一为可拖动、居中的应用风格弹窗。
